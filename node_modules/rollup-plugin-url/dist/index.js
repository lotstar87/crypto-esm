    ...
    load(id) {
      ...
      return Promise.all([promise(fs.stat, id), promise(fs.readFile, id)]).then(([stats, buffer]) => {
        ...
        if (limit && stats.size > limit || limit === 0) {
          ...
          const outputFileName = fileName.replace(/\[hash\]/g, hash).replace(/\[extname\]/g, ext).replace(/\[dirname\]/g, `${relativeDir}${path.sep}`).replace(/\[name\]/g, name);
